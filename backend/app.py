...
def init_db():
    ...
    db.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS questions (
            ...
            enable_python INTEGER DEFAULT 0,
    ...
    for col in ['enable_stata', 'enable_ai', 'has_attachment', 'enable_python']:
        try:
...
def download_exam_template():
    ...
    cell.value = (
        ...
        "9. AI列：下拉选择[是/否]，默认[是]。\n"
        "10. Python列：下拉选择[是/否]，默认[否]。开启后学生可在线编写和执行Python代码。\n"
        "11. 数据列：填写该题需要的数据文件名(.dta/.xlsx/.csv/.py)，多个用;隔开。数据文件通过附件管理上传。"
    )
    cell.font = note_font
    cell.alignment = note_align
    ws.row_dimensions[1].height = 220

    # v5.2: 14 columns (add Python column after AI)
    ws.merge_cells('A1:N1')
    headers = ['题型', '题干', '选项A', '选项B', '选项C', '选项D', '参考答案', '分值', '附件', '备注', '数据', 'Stata', 'AI', 'Python']
    ...
    widths = {'A': 14, 'B': 40, 'C': 18, 'D': 18, 'E': 18, 'F': 18,
              'G': 12, 'H': 10, 'I': 8, 'J': 35, 'K': 12, 'L': 8, 'M': 8, 'N': 8}
    for col, w in widths.items():
        ws.column_dimensions[col].width = w

    examples = [
        ('单选题', '以下哪个是Python的特点？', '编译型', '解释型', '汇编', '机器码', 'B', 5, '', '', '', '是', '是', '否'),
        ('多选题', '以下哪些是Web前端技术？', 'HTML', 'CSS', 'Python', 'JavaScript', 'ABD', 5, '', '', '', '是', '是', '否'),
        ('判断题', 'Python是一种面向对象的语言。', '', '', '', '', '正确', 5, '', '', '', '是', '否', '否'),
        ('简答题', '简述Python中的装饰器是什么。', '', '', '', '', '', 10, '', '', '', '是', '是', '否'),
        ('综合题', '阅读以下材料并回答问题...', '', '', '', '', '', 20, '是', 'chart.png', 'data1.dta', '否', '是', '是'),
    ]
    ...
    # v5.2: Python dropdown
    dv_python = DataValidation(type='list', formula1='"是,否"', allow_blank=True, showDropDown=False)
    dv_python.error = '请选择是/否'
    ws.add_data_validation(dv_python)
    for r in range(3, 105):
        dv_python.add(ws.cell(row=r, column=14))
...
def upload_exam_template():
    ...
    for row in rows:
        ...
        # v5.2: Read Python column (now at 13)
        # Forward-compatible: old templates (13 cols) won't have this column → default to 否
        enable_python_val = str(row[13]).strip() if len(row) > 13 and row[13] else '否'
        enable_python = 1 if enable_python_val in ['是', 'yes', '1'] else 0

        db.execute(
            "INSERT INTO questions (exam_id, q_type, content, option_a, option_b, option_c, option_d, correct_answer, score, sort_order, enable_stata, enable_ai, enable_python, has_attachment, remark, data_file) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (exam_id, q_type, content,
             ...
             score,
             sort_order, enable_stata, enable_ai, enable_python, has_attachment,
...
def start_exam(exam_id):
    ...
    rows = db.execute(
        ...
        "enable_stata, enable_ai, enable_python, has_attachment, remark, data_file "
...
def get_exam_questions_settings(exam_id):
    ...
    rows = db.execute("""
        SELECT q_id, q_type, content, score, sort_order,
               enable_stata, enable_ai, enable_python, has_attachment
        FROM questions WHERE exam_id = ? ORDER BY sort_order
    """, (exam_id,)).fetchall()
    return jsonify([dict(r) for r in rows])
...
def update_question_settings(exam_id):
    ...
    data = request.get_json()
    settings = data.get('settings', [])  # [{q_id, enable_stata, enable_ai, enable_python, has_attachment}]
    db = get_db()
    for s in settings:
        db.execute("""
            UPDATE questions SET enable_stata = ?, enable_ai = ?, enable_python = ?, has_attachment = ?
            WHERE q_id = ? AND exam_id = ?
        """, (
            int(s.get('enable_stata', 0)),
            int(s.get('enable_ai', 0)),
            int(s.get('enable_python', 0)),
            s['q_id'], exam_id
        ))
    db.commit()
    return jsonify({'ok': True})
...
def get_exam_preview(exam_id):
    ...
    rows = db.execute(
        "SELECT q_id, q_type, content, option_a, option_b, option_c, option_d, "
        "score, sort_order, enable_stata, enable_ai, enable_python, has_attachment, remark "
        "FROM questions WHERE exam_id = ? ORDER BY sort_order",
        (exam_id,)
    ).fetchall()
...
# ════════════════════════════════════════════
# Python Execution API (v5.2)
# ════════════════════════════════════════════

@app.route('/api/python/execute', methods=['POST'])
def execute_python_command():
    """Execute Python code on server and return results.
    Uses subprocess for clean, contained execution.
    Accepts 'exam_id' + 'session_id' to cd to the student's per-session workdir.
    """
    data = request.get_json()
    code = data.get('code', '').strip()
    if not code:
        return jsonify({'error': '请输入Python代码'}), 400
    ...
    try:
        # Use subprocess to run python with the code
        import subprocess
        import sys
        ...
        return jsonify({'error': 'Python代码执行超时（超过30秒）'}), 504
    except Exception as e:
        return jsonify({'error': f'Python执行失败: {str(e)}'}), 500
