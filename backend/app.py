...
def _deploy_exam_data(exam_id: int):
    """Copy data files (dta/xlsx) referenced by questions to the exam workdir."""
    db = get_db()
    exam = db.execute(
    ...
    workdir = _exam_workdir(username, exam_id)
    ...
    for fname in data_files:
        ...
        for src_dir in [teacher_attach_dir, exam_attach_dir]:
            ...
            if os.path.exists(src_path):
                try:
                    shutil.copy2(src_path, os.path.join(workdir, fname))
                    copied = True
                    break
                except Exception:
                    pass
...
def _cleanup_exam_workdir(exam_id: int):
    """Delete the exam's working directory for data files."""
    db = get_db()
    exam = db.execute(
...
def _execute_stata_via_mcp(command_text: str) -> str:
    ...
    try:
        ...
        # stata_do returns log content directly
        # Clean up the log: remove blank lines at start/end, truncate to 5000 chars
        if result:
            lines = result.split('\n')
...
def upload_exam_template():
    ...
    for row in rows:
        ...
        enable_stata_val = str(row[11]).strip() if len(row) > 11 and row[11] else '是'
        enable_ai_val = str(row[12]).strip() if len(row) > 12 and row[12] else '是'
...
def student_login():
    ...
    # No longer check Stata (PlanB: server-side execution)
...
def start_exam(exam_id):
    ...
    # v4.2: Deploy data files to exam workdir
    _deploy_exam_data(exam_id)
    ...
    # Resolve teacher username
    teacher_username = exam_info.get('teacher_username', '')
    workdir_hint = ''
    if teacher_username:
        workdir_hint = _exam_workdir(teacher_username, exam_id)
...
def submit_exam(session_id):
    ...
    # v4.2: Lazy cleanup — if all students have submitted, remove workdir
    try:
...
def save_exam_settings(exam_id):
    ...
    db.execute("""
        UPDATE exams SET
            exam_title = ?, school = ?, college = ?, class_name = ?, grade = ?,
            exam_duration = ?, exam_start_time = ?, exam_notice = ?, exam_notice_title = ?,
            exam_late_start_limit = ?, exam_late_submit_limit = ?,
            anti_shuffle_questions = ?, anti_shuffle_options = ?, anti_screen_switch = ?
        WHERE exam_id = ?
...
def execute_stata_command():
    ...
    Accepts optional 'exam_id' to set working directory to the exam workdir.
    """
    data = request.get_json()
    ...
    # Set working directory if exam_id is provided
    ...
    try:
        log_content = _execute_stata_via_mcp(command)
        return jsonify({
            'ok': True,
            'output': log_content
        })
